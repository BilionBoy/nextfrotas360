...
class AStatusController < ApplicationController
  ...
  def create
    ...
    if @a_status.save
      redirect_to a_status_index_path, notice: t('messages.created_successfully')
    else
  ...
  def update
    if @a_status.update(a_status_params)
      redirect_to a_status_index_path, notice: t('messages.updated_successfully'), status: :see_other
    else
  ...
  def set_a_status
    ...
    return redirect_to a_status_index_path, alert: t('messages.not_found') unless @a_status
  end
  ...
  def handle_not_found
    redirect_to a_status_index_path, alert: t('messages.not_found')
  end
end
