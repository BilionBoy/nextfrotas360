...
class GVeiculosController < ApplicationController
  ...
  def update
    if @g_veiculo.update(g_veiculo_params)
      redirect_to g_veiculos_path, notice: t('messages.updated_successfully'), status: :see_other
    else
  ...
  def handle_not_found
    redirect_to g_veiculos_path, alert: t('messages.not_found')
  end
end
